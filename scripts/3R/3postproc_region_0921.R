...
root_folder <- 'C:/Users/wyang80/Desktop/ProtConn'
results_folder <- 'C:/Users/wyang80/Desktop/ProtConn/results'
results_folder1 <- 'C:/Users/wyang80/Desktop/ProtConn/results/test1'
results_folder2 <- 'C:/Users/wyang80/Desktop/ProtConn/results/test2'
results_folder3 <- 'C:/Users/wyang80/Desktop/ProtConn/results/test3'
results_folder6 <- 'C:/Users/wyang80/Desktop/ProtConn/results/test6'
results_folder7 <- 'C:/Users/wyang80/Desktop/ProtConn/results/test7'
...
table_with_trans <- paste(results_folder7,"/results_countries_EC_PC_with_trans.txt", sep = "")
table_without_trans <- paste(results_folder7,"/results_countries_EC_PC_without_trans.txt", sep = "")
table_ECA_min_max_with_trans <- paste(results_folder7,"/ECAminmax_countries_with_trans.txt", sep = "")
table_ECA_min_max_without_trans <- paste(results_folder7,"/ECAminmax_countries_without_trans.txt", sep = "")
...
table_for_BOUND <- paste(results_folder7, "/results_EC_PC_bound_corr.txt", sep = "")
table_for_BOUND_wr <- paste(results_folder7, "/results_EC_PC_bound_corr_wr.txt", sep = "")
table_for_PA_nds <- paste(results_folder, '/STATE_attrib_table_PA_mexcanus_nds_july2021.csv', sep = "") # if generated from script, will be a txt file
table_for_PA_ds <- paste(results_folder, '/STATE_attrib_table_PA_mexcanus_final_july2021.csv', sep = "")
...
table_country_area <- 'C:/Users/wyang80/Desktop/ProtConn/results/STATE_area.txt'

## Output files
outfile_all_results <- paste(results_folder,"/All_values_STATE_final_",'test4',".txt", sep = "")
outfile_protconn_results <- paste(results_folder,"/ProtConn_results_STATE_final_",'test4',".txt", sep = "")
...
data_within_and_outside <- data_within_and_outside[!duplicated(data_within_and_outside),]
...
data_PA_nds <- na.omit(read.csv(table_for_PA_nds, header = TRUE))
data_PA_nds <- data_PA_nds[data_PA_nds$ifPA == 1, ]
#data_PA_nds$AREA_nds <- data_PA_nds$Shape_Area/1000000
data_PA_nds$AREA_nds <- data_PA_nds$AREA_GEO
...
data_PA_ds <- na.omit(read.csv(table_for_PA_ds) %>% select(ifPA, STATEFP, AREA_GEO))
data_PA_ds <- data_PA_ds[data_PA_ds$ifPA == 1, ]
data_PA_ds$ifPA <- NULL
#data_PA_ds$AREA_ds <- data_PA_ds$Shape_Area/1000000
data_PA_ds$AREA_ds <- data_PA_ds$AREA_GEO
...
data_within_only <- data_within_only[!grepl("NA", data_within_only$Prefix),]
data_within_and_outside$Prefix <- gsub('_test', '', data_within_and_outside$Prefix)
data_within_and_outside <- data_within_and_outside[!grepl("NA", data_within_and_outside$Prefix),]
data_ECA_min_max_within_and_outside$Prefix <- gsub('_test', '', data_ECA_min_max_within_and_outside$Prefix)
data_ECA_min_max_within_and_outside <- data_ECA_min_max_within_and_outside[!grepl("NA", data_ECA_min_max_within_and_outside$Prefix),]
# data_ECA_min_max_within_and_outside$Prefix <- gsub('_te', '', data_ECA_min_max_within_and_outside$Prefix)
data_ECA_min_max_within_only$Prefix <- gsub('_test_WITHOUT_TRANS', '', data_ECA_min_max_within_only$Prefix)
data_ECA_min_max_within_only <- data_ECA_min_max_within_only[!grepl("NA", data_ECA_min_max_within_only$Prefix),]
...
sum(merge_data_ECA_min_max$dfi_max*merge_data_ECA_min_max$dif_max)
# these four lines ask that ECA min max be the same for within the country and within and with transboundary neighbors considered.
...
data_within_only <- data_within_only[order(data_within_only$Prefix),]
merge_data_within_only_and_ECA_min <- merge_data_within_only_and_ECA_min[order(merge_data_within_only_and_ECA_min$Prefix),]
data_within_only$EC.PC. <- pmax(data_within_only$EC.PC.,merge_data_within_only_and_ECA_min$V2)
data_within_and_outside <- data_within_and_outside[order(data_within_and_outside$Prefix),]
merge_data_within_and_outside_and_ECA_min <- merge_data_within_and_outside_and_ECA_min[order(merge_data_within_and_outside_and_ECA_min$Prefix),]
...
data_for_BOUND <- data_for_BOUND[!grepl("NA", data_for_BOUND$Prefix),]
...
data_for_BOUND_wr <- data_for_BOUND_wr[!grepl("NA", data_for_BOUND_wr$Prefix),]
...
data_country_area <- data_country_area %>% select(STATEFP, Shape_Area)
...
#merge_all_data <- merge_all_data[merge_all_data$Prefix == 15,]
...
#X ProtConn[Unprot] = ProtConn - ProtConn[Prot] - ProtConn[Trans]
#Y ProtConn[Unprot] = 100 - ProtConn[Prot] - ProtConn[Trans]
...
merge_all_data$ProtConn_Unprot <- 100 - merge_all_data$ProtConn_Prot - merge_all_data$ProtConn_Trans
...
colnames(All_results_final)[1] <- c("STATEFP")
...
colnames(ProtConn_results_final)[1] <- c("STATEFP")
