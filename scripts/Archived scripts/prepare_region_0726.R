...
root_folder <- 'C:\\Users\\ywx12\\Desktop\\PhD\\RA\\ProtectedLand'
# the following two folders are to be created
node_dist_files_300_km_folder <- 'C:\\Users\\ywx12\\Desktop\\PhD\\RA\\ProtectedLand\\tmp1'
node_dist_files_without_trans_300_km_folder <- 'C:\\Users\\ywx12\\Desktop\\PhD\\RA\\ProtectedLand\\tmp1_notrans'

## This is the distance file calculated from the protected area (PA) layer for the countries
raw_distance_file <-  'C:\\Users\\ywx12\\Desktop\\PhD\\RA\\ProtectedLand\\near_tab_test.csv'
...
raw_nodes_file <-  'C:\\Users\\ywx12\\Desktop\\PhD\\RA\\ProtectedLand\\node_test.csv'

## Variables for naming nodes and distance files
prefix_nodes_files <- "nodes_"
prefix_distances_files <- "distances_"

out_ec <- paste(node_dist_files_300_km_folder,"/ECAminmax_countries_with_trans.txt", sep = "")
out_ec_wt <- paste(node_dist_files_without_trans_300_km_folder,"/ECAminmax_countries_without_trans.txt", sep = "")
...
directory_name <- paste(node_dist_files_300_km_folder,"/",sep="")
directory_name_WITHOUT_TRANS <- paste(node_dist_files_without_trans_300_km_folder,"/",sep="")
...
for (i in list_id)
{
  ...
  temp_Distances_current_county <- Distances[Distances$IN_FID %in% Nodes_current_county$OBJECTID | Distances$NEAR_FID %in% Nodes_current_county$OBJECTID,]
  ...
  temp_Nodes_FINAL_current_county <- Nodes[Nodes$OBJECTID %in% temp_Distances_current_county$IN_FID  | Nodes$OBJECTID %in% temp_Distances_current_county$NEAR_FID | Nodes$OBJECTID %in% Nodes_current_county$OBJECTID,]
  ...
  Distances_current_county <- Distances[Distances$IN_FID %in% temp_Nodes_FINAL_current_county$OBJECTID | Distances$NEAR_FID %in% temp_Nodes_FINAL_current_county$OBJECTID,]
  ...
  Distances_current_county_WITHOUT_TRANS <- Distances[Distances$IN_FID %in% Nodes_current_county$OBJECTID & Distances$NEAR_FID %in% Nodes_current_county$OBJECTID,]
  ...
  Nodes_FINAL_current_county <- Nodes[Nodes$OBJECTID %in% Distances_current_county$IN_FID  | Nodes$OBJECTID %in% Distances_current_county$NEAR_FID | Nodes$OBJECTID %in% Nodes_current_county$OBJECTID,]
...
file_names_nodes <- dir(node_dist_files_300_km_folder, pattern = prefix_nodes_files)
file_names_distances <- dir(node_dist_files_300_km_folder, pattern = prefix_distances_files)
...
file_names_nodes_wt <- dir(node_dist_files_without_trans_300_km_folder, pattern = prefix_nodes_files)
file_names_distances_wt <- dir(node_dist_files_without_trans_300_km_folder, pattern = prefix_distances_files)
