...
table_with_trans <- paste(args[6],"/results_countries_EC_PC_with_trans.txt", sep = "")
table_without_trans <- paste(args[6],"/results_countries_EC_PC_without_trans.txt", sep = "")
table_ECA_min_max_with_trans <- paste(args[6],"/ECAminmax_countries_with_trans.txt", sep = "")
...
## ALA (small Finnish islands close to Sweden) is an ISO3 code that appears in the WDPA of 2018 and later; in earlier WDPA versions the PAs in these islands were coded as FIN 
## (Finland). We make the area adjustment considering the total land area of 1580 km2 for ALA according to Protected Planet
## Updated on 20191202 (area in wdpa_201911=792)
merge_all_data$areakm2[merge_all_data$Prefix == "ALA"] <- 792
merge_all_data$areakm2[merge_all_data$Prefix == "FIN"] <- merge_all_data$areakm2[merge_all_data$Prefix == "FIN"] -792
...
## Updated on 20191202 (area in wdpa_201911=515)
...
## Updated on 20191202 (area in wdpa_201911=2839)
...
## Updated on 20191202 (area in wdpa_201911=87)
merge_all_data$areakm2[merge_all_data$Prefix == "CUW"] <- 87
merge_all_data$areakm2[merge_all_data$Prefix == "ANT"] <- merge_all_data$areakm2[merge_all_data$Prefix == "ANT"] - 87
...
## Updated on 20191202 (area in wdpa_201911=110)
