...
for TIL in $(for i in $(eval echo {0..$NCORES}); do ((start=${TILESIZE}*$i)); echo -n $start" "; done)
	...
	do
		eco_idcode=${PA:6:5}
		eco_suffix="${PA:6:5}"
		echo "./conefor2.7.3Linux -nodeFile nodes_${eco_idcode}_2019_WITHOUT_TRANS -conFile distances_${eco_idcode}_2019_WITHOUT_TRANS -prefix "${eco_suffix}"_2019_WITHOUT_TRANS -t dist -confProb 10000 0.5 -PC -F -AWF onlyoverall"
	done
...
(head -n 1 "results_all_EC(PC).txt" && tail -n +2 "results_all_EC(PC).txt" | sort) > ${results_folder}/results_ecoregion_without_trans.txt
