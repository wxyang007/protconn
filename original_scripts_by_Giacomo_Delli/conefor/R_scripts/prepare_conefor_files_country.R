...
root_folder <- args[11]
node_dist_files_300_km_folder <- args[6]
node_dist_files_without_trans_300_km_folder <- args[7]
...
out_ec <- paste(args[6],"/ECAminmax_countries_with_trans.txt", sep = "")
out_ec_wt <- paste(args[7],"/ECAminmax_countries_without_trans.txt", sep = "")
...
end_file_name <- paste("_",args[10], sep = "")
end_file_name_WITHOUT_TRANS <- paste("_",args[10],"_WITHOUT_TRANS",sep = "")
...
for(i in 1:length(file_names_nodes_wt))
{
  ...
  ECAminmax[i,1]=substr(file_names_nodes_wt[i],nchar(prefix_nodes_files)+1,nchar(prefix_nodes_files)+3+nchar(end_file_name))
  ECAminmax[i,2]=sqrt(sum(nodefile[,2]*nodefile[,2]))
  ECAminmax[i,3]=sum(nodefile[,2])
  if (is.null(nrow(distfile)))
  {
