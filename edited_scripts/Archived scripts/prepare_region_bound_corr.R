...
node_dist_files_for_bound_folder <- 'C:\\Users\\ywx12\\Desktop\\PhD\\RA\\ProtectedLand\\tmp1_bound'
node_dist_files_for_bound_wr_folder <- 'C:\\Users\\ywx12\\Desktop\\PhD\\RA\\ProtectedLand\\tmp1_bound_wr'
...
## I saved only OBJECTID, GEOID, and shape_area
...
for (i in list_id)
{
  ...
  temp_2_Distances_current_county <- Distances[Distances$IN_FID %in% temp_Nodes_FINAL_current_county$OBJECTID | Distances$NEAR_FID %in% temp_Nodes_FINAL_current_county$OBJECTID,]
  ## And get the set of nodes to be considered at this point
  temp_2_Nodes_FINAL_current_county <- Nodes[Nodes$OBJECTID %in% temp_2_Distances_current_county$IN_FID  | Nodes$OBJECTID %in% temp_2_Distances_current_county$NEAR_FID | Nodes$OBJECTID %in% Nodes_current_county$OBJECTID,]
  
  temp_2_Distances_current_county_wr <- Distances[Distances$IN_FID %in% temp_Nodes_FINAL_current_county_wr$OBJECTID | Distances$NEAR_FID %in% temp_Nodes_FINAL_current_county_wr$OBJECTID,]
  temp_2_Nodes_FINAL_current_county_wr <- Nodes[Nodes$OBJECTID %in% temp_2_Distances_current_county_wr$IN_FID | Nodes$OBJECTID %in% temp_2_Distances_current_county_wr$NEAR_FID | Nodes$OBJECTID %in% Nodes_current_county$OBJECTID,]
  ...
  Distances_FINAL_current_county_wr <- Distances[Distances$IN_FID %in% Nodes_FINAL_current_county_wr$OBJECTID & Distances$NEAR_FID %in% Nodes_FINAL_current_county_wr$OBJECTID,]
  ...
  Distances_FINAL_current_county <- Distances[Distances$IN_FID %in% Nodes_FINAL_current_county$OBJECTID & Distances$NEAR_FID %in% Nodes_FINAL_current_county$OBJECTID,]
