...
node_dist_files_for_bound_folder <- 'C:/Users/wyang80/Desktop/ProtConn/STATES/state_files_bound_test4'
node_dist_files_for_bound_wr_folder <- 'C:/Users/wyang80/Desktop/ProtConn/STATES/state_files_bound_wr_test4'
...
Nodes$STATEFP <- NULL
Nodes$AREA_GEO[Nodes$ifPA != 1] <- 0
Nodes$AREA_GEO[is.na(Nodes$GEOID)] <- 0
...
# Nodes$ifPA <- NULL
Nodes <- Nodes[c("ORIG_FID", "GEOID", "AREA_GEO", "ifPA")]


list_id <- unique(Nodes$GEOID)
list_id <- list_id[list_id != "NA"]
...
for (i in list_id)
{
  ...
  Nodes_current_county <- Nodes[Nodes$GEOID %in% i & Nodes$ifPA == 1,]
  ...
  # n + t + q
  temp_Nodes_FINAL_current_county <- temp_Nodes_FINAL_current_county[!(temp_Nodes_FINAL_current_county$ifPA==0 & !(temp_Nodes_FINAL_current_county$GEOID %in% i)),]
  ...
  Nodes_FINAL_current_county <- temp_2_Nodes_FINAL_current_county[!(temp_2_Nodes_FINAL_current_county$ifPA==0 & !(temp_2_Nodes_FINAL_current_county$GEOID %in% i)),]
  ...
  Nodes_FINAL_current_county_wr$AREA_GEO[is.na(Nodes_FINAL_current_county_wr$GEOID)] <- 0
  ...
  Nodes_FINAL_current_county_wr$ifPA <- NULL
  Nodes_FINAL_current_county$GEOID <- NULL
  Nodes_FINAL_current_county$ifPA <- NULL
...
for(i in 1:length(file_names_nodes))
{
  ...
  #nodefile <- read.table("nodes_60_test",header=FALSE, sep="\t", stringsAsFactors=FALSE)
...
{
  file_names_nodes_wr[i] = "nodes_60_test_wr"
  ...
  if (is.null(nrow(distfile)))
  {
    nodefile[nrow(nodefile)+1,] =c(-1,0)
    distfile <-as.table(matrix(c(nodefile[1,1],nodefile[nrow(nodefile),1],99999999999999999999),ncol=3,byrow=TRUE))
    write.table(nodefile, file = file_names_nodes_wr[i], row.names=FALSE, col.names=FALSE, sep="\t")
    write.table(distfile, file=paste(prefix_distances_files, substr(file_names_nodes_wr[i], nchar(prefix_nodes_files)+1, nchar(file_names_nodes_wr[i])),sep=""),row.names=FALSE,col.names = FALSE,sep="\t")
  }
}
## Now we write the minimum and maximum values that are possible for ECA in each country
