...
-nln "${protconn_schema}.wdpa_plus_land_flat_1km2_final" \
-nlt "MULTIPOLYGON" \
${DATADIR}/wdpa_plus_land_flat_1km_final.shp

wait
end=`date +%s`
runtime=$(((end-start) / 60))
echo "shapefile file imported in PG as table "${protconn_schema}".wdpa_plus_land_flat_1km2_final in "${runtime}" minutes"
...
start=`date +%s`
psql ${dbpar} -t -v OUTSCHEMA=${protconn_schema} -f ${SQLDIR}/generate_near_table_country_boundcorr.sql
end=`date +%s`
runtime=$(((end-start) / 60))
wait
...
echo "\copy ${protconn_schema}.all_distances_wdpa_plus_land_100km TO '${DATADIR}/All_distances_pg_WDPA_plus_LAND100km_Mar2020.txt' delimiter ',' csv HEADER" > ${SQLDIR}/export_near_table_to_txt.sql
