...
echo " "
echo "-----------------------------------------------------------------------------------"
echo "--- This scripts executes in parallel the connectivity analysis                 ---"
echo "--- for bound correction                                                        ---"
...
(head -n 1 "results_all_EC(PC).txt" && tail -n +2 "results_all_EC(PC).txt" | sort) > ${results_folder}/results_all_EC_PC_boundcorr.txt
